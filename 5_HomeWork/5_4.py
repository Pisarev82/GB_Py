...
input_str = """eeeeellk wrrrriiiffgj wrrrgggrsssrrrrrsssf 
rioiiiiiii iiprrrmkkkk jkjjjjjjjrrrrrrr
ааалллллыххх аваоцукепо вчеорыкоо"""

def encoded_to_str (input_str):
    encoded_str_1 = input_str.split()
    # lambda x : re.match('x', 'superstition').span()
    encoded_str_1 = [str.count(str(encoded_str_1), x) for x in encoded_str_1]
...
encoded_str = encoded_to_str (input_str)
print(encoded_to_str (input_str))
...
#         result += int(text_list[i])*text_list2[i]
#     return result
